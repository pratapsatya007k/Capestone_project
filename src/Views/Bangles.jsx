import React from 'react'
  import { useState } from 'react'
  import "../public/gold.css"
  import Header from './Header'
  import img from "../public/wish.png"
  import  axios  from 'axios'
  import searchstyle from '../public/search.module.css'
  import Search from './search'
const Bangles = () => {
  const user=localStorage.getItem("username");
  const [blur,setblur]=useState("0px");
  const [hide,sethide]=useState("none");
  function Searchopen(){
    sethide("inline");
    setblur("5px");
  }
  function wrongsearch(){
    sethide("none");
    setblur("0px")
  }
  const [src1,setSrc1]=useState("https://imgs.search.brave.com/mi-zvLzHi8N0-s_eGNrU-g5QE4sq30M0inIQ5dMJvac/rs:fit:500:0:0/g:ce/aHR0cHM6Ly91cGxv/YWQud2lraW1lZGlh/Lm9yZy93aWtpcGVk/aWEvY29tbW9ucy8x/LzE4L0ZpdmUtcG9p/bnRlZF9zdGFyLnN2/Zw.svg");
    const [src2,setSrc2]=useState("https://imgs.search.brave.com/mi-zvLzHi8N0-s_eGNrU-g5QE4sq30M0inIQ5dMJvac/rs:fit:500:0:0/g:ce/aHR0cHM6Ly91cGxv/YWQud2lraW1lZGlh/Lm9yZy93aWtpcGVk/aWEvY29tbW9ucy8x/LzE4L0ZpdmUtcG9p/bnRlZF9zdGFyLnN2/Zw.svg");
    const [src3,setSrc3]=useState("https://imgs.search.brave.com/mi-zvLzHi8N0-s_eGNrU-g5QE4sq30M0inIQ5dMJvac/rs:fit:500:0:0/g:ce/aHR0cHM6Ly91cGxv/YWQud2lraW1lZGlh/Lm9yZy93aWtpcGVk/aWEvY29tbW9ucy8x/LzE4L0ZpdmUtcG9p/bnRlZF9zdGFyLnN2/Zw.svg");
    const [src4,setSrc4]=useState("https://imgs.search.brave.com/mi-zvLzHi8N0-s_eGNrU-g5QE4sq30M0inIQ5dMJvac/rs:fit:500:0:0/g:ce/aHR0cHM6Ly91cGxv/YWQud2lraW1lZGlh/Lm9yZy93aWtpcGVk/aWEvY29tbW9ucy8x/LzE4L0ZpdmUtcG9p/bnRlZF9zdGFyLnN2/Zw.svg")
    const [src5,setSrc5]=useState("https://imgs.search.brave.com/mi-zvLzHi8N0-s_eGNrU-g5QE4sq30M0inIQ5dMJvac/rs:fit:500:0:0/g:ce/aHR0cHM6Ly91cGxv/YWQud2lraW1lZGlh/Lm9yZy93aWtpcGVk/aWEvY29tbW9ucy8x/LzE4L0ZpdmUtcG9p/bnRlZF9zdGFyLnN2/Zw.svg");
    const [src6,setSrc6]=useState("https://imgs.search.brave.com/mi-zvLzHi8N0-s_eGNrU-g5QE4sq30M0inIQ5dMJvac/rs:fit:500:0:0/g:ce/aHR0cHM6Ly91cGxv/YWQud2lraW1lZGlh/Lm9yZy93aWtpcGVk/aWEvY29tbW9ucy8x/LzE4L0ZpdmUtcG9p/bnRlZF9zdGFyLnN2/Zw.svg");
    const [src7,setSrc7]=useState("https://imgs.search.brave.com/mi-zvLzHi8N0-s_eGNrU-g5QE4sq30M0inIQ5dMJvac/rs:fit:500:0:0/g:ce/aHR0cHM6Ly91cGxv/YWQud2lraW1lZGlh/Lm9yZy93aWtpcGVk/aWEvY29tbW9ucy8x/LzE4L0ZpdmUtcG9p/bnRlZF9zdGFyLnN2/Zw.svg");
    const [src8,setSrc8]=useState("https://imgs.search.brave.com/mi-zvLzHi8N0-s_eGNrU-g5QE4sq30M0inIQ5dMJvac/rs:fit:500:0:0/g:ce/aHR0cHM6Ly91cGxv/YWQud2lraW1lZGlh/Lm9yZy93aWtpcGVk/aWEvY29tbW9ucy8x/LzE4L0ZpdmUtcG9p/bnRlZF9zdGFyLnN2/Zw.svg");
    const [src9,setSrc9]=useState("https://imgs.search.brave.com/mi-zvLzHi8N0-s_eGNrU-g5QE4sq30M0inIQ5dMJvac/rs:fit:500:0:0/g:ce/aHR0cHM6Ly91cGxv/YWQud2lraW1lZGlh/Lm9yZy93aWtpcGVk/aWEvY29tbW9ucy8x/LzE4L0ZpdmUtcG9p/bnRlZF9zdGFyLnN2/Zw.svg");
    let cnt1=0;
    let cnt2=0;
    let cnt3=0;
    let cnt4=0;
    let cnt5=0;
    let cnt6=0;
    let cnt7=0;
    let cnt8=0;
    let cnt9=0;
    function starFn1(weight,total){
      if(cnt1%2===0){
      setSrc1(img);
      cnt1++;
      axios.post('/wish/insert', {
        imgsrc : "https://www.lalithaajewellery.com/assets/img/bangles/1.webp",
        itemName : "Gold Bangles",
        itemid:"B1",
        username:user,
        itemType:"Gold",
        weight:weight,
        total:total
      });

      }else{
        setSrc1("https://imgs.search.brave.com/mi-zvLzHi8N0-s_eGNrU-g5QE4sq30M0inIQ5dMJvac/rs:fit:500:0:0/g:ce/aHR0cHM6Ly91cGxv/YWQud2lraW1lZGlh/Lm9yZy93aWtpcGVk/aWEvY29tbW9ucy8x/LzE4L0ZpdmUtcG9p/bnRlZF9zdGFyLnN2/Zw.svg");
        cnt1++;
        const key="itemid";
        const value="B1";
        try {
         axios.delete('/wish/delete', { data: { key:key, value:value,username:user} });
          // Handle successful deletion
          console.log('Data deleted successfully.');
        } catch (error) {
          // Handle error
          console.error('Error deleting data:', error);
        }
      }
    }
    function starFn2(weight,total){
      if(cnt2%2===0){
      setSrc2(img);
      cnt2++;
      axios.post('/wish/insert', {
        imgsrc : "https://www.lalithaajewellery.com/assets/img/bangles/2.webp",
        itemName : "Gold Bangles",
        itemid:"B2",
        username:user,
        itemType:"Gold",
        weight:weight,
        total:total
      });
      }else{
        setSrc2("https://imgs.search.brave.com/mi-zvLzHi8N0-s_eGNrU-g5QE4sq30M0inIQ5dMJvac/rs:fit:500:0:0/g:ce/aHR0cHM6Ly91cGxv/YWQud2lraW1lZGlh/Lm9yZy93aWtpcGVk/aWEvY29tbW9ucy8x/LzE4L0ZpdmUtcG9p/bnRlZF9zdGFyLnN2/Zw.svg");
        cnt2++;
        const key="itemid";
        const value="B2";
        try {
         axios.delete('/wish/delete', { data: { key:key, value:value,username:user} });
          // Handle successful deletion
          console.log('Data deleted successfully.');
        } catch (error) {
          // Handle error
          console.error('Error deleting data:', error);
        }
      }
    }
    function starFn3(weight,total){
      if(cnt3%2===0){
      setSrc3(img);
      cnt3++
      axios.post('/wish/insert', {
        imgsrc : "https://www.lalithaajewellery.com/assets/img/bangles/3.webp",
        itemName : "Gold Bangles",
        itemid:"B3",
        username:user,
        itemType:"Gold",
        weight:weight,
        total:total
      });
      }else{
        setSrc3("https://imgs.search.brave.com/mi-zvLzHi8N0-s_eGNrU-g5QE4sq30M0inIQ5dMJvac/rs:fit:500:0:0/g:ce/aHR0cHM6Ly91cGxv/YWQud2lraW1lZGlh/Lm9yZy93aWtpcGVk/aWEvY29tbW9ucy8x/LzE4L0ZpdmUtcG9p/bnRlZF9zdGFyLnN2/Zw.svg");
        cnt3++
        const key="itemid";
        const value="B3";
        try {
         axios.delete('/wish/delete', { data: { key:key, value:value,username:user} });
          // Handle successful deletion
          console.log('Data deleted successfully.');
        } catch (error) {
          // Handle error
          console.error('Error deleting data:', error);
        }
      }
    }
    function starFn4(weight,total){
      if(cnt4%2===0){
      setSrc4(img);
      cnt4++
      axios.post('/wish/insert', {
        imgsrc : "https://www.lalithaajewellery.com/assets/img/bangles/4.webp",
        itemName : "Gold Bangles",
        itemid:"B4",
        username:user,
        itemType:"Gold",
        weight:weight,
        total:total
      });
      }else{
        setSrc4("https://imgs.search.brave.com/mi-zvLzHi8N0-s_eGNrU-g5QE4sq30M0inIQ5dMJvac/rs:fit:500:0:0/g:ce/aHR0cHM6Ly91cGxv/YWQud2lraW1lZGlh/Lm9yZy93aWtpcGVk/aWEvY29tbW9ucy8x/LzE4L0ZpdmUtcG9p/bnRlZF9zdGFyLnN2/Zw.svg");
        cnt4++
        const key="itemid";
        const value="B4";
        try {
         axios.delete('/wish/delete', { data: { key:key, value:value,username:user} });
          // Handle successful deletion
          console.log('Data deleted successfully.');
        } catch (error) {
          // Handle error
          console.error('Error deleting data:', error);
        }
      }
    }
    function starFn5(weight,total){
      if(cnt5%2===0){
      setSrc5(img);
      cnt5++
      axios.post('/wish/insert', {
        imgsrc : "https://www.lalithaajewellery.com/assets/img/bangles/5.webp",
        itemName : "Gold Bangles",
        itemid:"B5",
        username:user,
        itemType:"Gold",
        weight:weight,
        total:total
      });
      }else{
        setSrc5("https://imgs.search.brave.com/mi-zvLzHi8N0-s_eGNrU-g5QE4sq30M0inIQ5dMJvac/rs:fit:500:0:0/g:ce/aHR0cHM6Ly91cGxv/YWQud2lraW1lZGlh/Lm9yZy93aWtpcGVk/aWEvY29tbW9ucy8x/LzE4L0ZpdmUtcG9p/bnRlZF9zdGFyLnN2/Zw.svg");
        cnt5++
        const key="itemid";
        const value="B5";
        try {
         axios.delete('/wish/delete', { data: { key:key, value:value,username:user} });
          // Handle successful deletion
          console.log('Data deleted successfully.');
        } catch (error) {
          // Handle error
          console.error('Error deleting data:', error);
        }
      }
    }
    function starFn6(weight,total){
      if(cnt6%2===0){
      setSrc6(img);
      cnt6++
      axios.post('/wish/insert', {
        imgsrc : "https://www.lalithaajewellery.com/assets/img/bangles/7.webp",
        itemName : "Gold Bangles",
        itemid:"B6",
        username:user,
        itemType:"Gold",
        weight:weight,
        total:total
      });
      }else{
        setSrc6("https://imgs.search.brave.com/mi-zvLzHi8N0-s_eGNrU-g5QE4sq30M0inIQ5dMJvac/rs:fit:500:0:0/g:ce/aHR0cHM6Ly91cGxv/YWQud2lraW1lZGlh/Lm9yZy93aWtpcGVk/aWEvY29tbW9ucy8x/LzE4L0ZpdmUtcG9p/bnRlZF9zdGFyLnN2/Zw.svg");
        cnt6++
        const key="itemid";
        const value="B6";
        try {
         axios.delete('/wish/delete', { data: { key:key, value:value,username:user} });
          // Handle successful deletion
          console.log('Data deleted successfully.');
        } catch (error) {
          // Handle error
          console.error('Error deleting data:', error);
        }
      }
    }
    function starFn7(weight,total){
      if(cnt7%2===0){
      setSrc7(img);
      cnt7++
      axios.post('/wish/insert', {
        imgsrc : "https://www.lalithaajewellery.com/assets/img/bangles/11.webp",
        itemName : "Gold Bangles",
        itemid:"B7",
        username:user,
        itemType:"Gold",
        weight:weight,
        total:total
      });
      }else{
        setSrc7("https://imgs.search.brave.com/mi-zvLzHi8N0-s_eGNrU-g5QE4sq30M0inIQ5dMJvac/rs:fit:500:0:0/g:ce/aHR0cHM6Ly91cGxv/YWQud2lraW1lZGlh/Lm9yZy93aWtpcGVk/aWEvY29tbW9ucy8x/LzE4L0ZpdmUtcG9p/bnRlZF9zdGFyLnN2/Zw.svg");
        cnt7++
        const key="itemid";
        const value="B7";
        try {
         axios.delete('/wish/delete', { data: { key:key, value:value,username:user} });
          // Handle successful deletion
          console.log('Data deleted successfully.');
        } catch (error) {
          // Handle error
          console.error('Error deleting data:', error);
        }
      }
    }
    function starFn8(weight,total){
      if(cnt8%2===0){
      setSrc8(img);
      cnt8++
      axios.post('/wish/insert', {
        imgsrc : "https://www.lalithaajewellery.com/assets/img/earring/10.webp",
        itemName : "Gold Bangles",
        itemid:"B8",
        username:user,
        itemType:"Gold",
        weight:weight,
        total:total
      });
      }else{
        setSrc8("https://imgs.search.brave.com/mi-zvLzHi8N0-s_eGNrU-g5QE4sq30M0inIQ5dMJvac/rs:fit:500:0:0/g:ce/aHR0cHM6Ly91cGxv/YWQud2lraW1lZGlh/Lm9yZy93aWtpcGVk/aWEvY29tbW9ucy8x/LzE4L0ZpdmUtcG9p/bnRlZF9zdGFyLnN2/Zw.svg");
        cnt8++
        const key="itemid";
        const value="B8";
        try {
         axios.delete('/wish/delete', { data: { key:key, value:value,username:user} });
          // Handle successful deletion
          console.log('Data deleted successfully.');
        } catch (error) {
          // Handle error
          console.error('Error deleting data:', error);
        }
      }
    }
    function starFn9(weight,total){
      if(cnt9%2===0){
      setSrc9(img);
      cnt9++
      axios.post('/wish/insert', {
        imgsrc : "https://www.lalithaajewellery.com/assets/img/bangles/12.webp",
        itemName : "Gold Bangles",
        itemid:"B9",
        username:user,
        itemType:"Gold",
        weight:weight,
        total:total
      });
      }else{
        setSrc9("https://imgs.search.brave.com/mi-zvLzHi8N0-s_eGNrU-g5QE4sq30M0inIQ5dMJvac/rs:fit:500:0:0/g:ce/aHR0cHM6Ly91cGxv/YWQud2lraW1lZGlh/Lm9yZy93aWtpcGVk/aWEvY29tbW9ucy8x/LzE4L0ZpdmUtcG9p/bnRlZF9zdGFyLnN2/Zw.svg");
        cnt9++
        const key="itemid";
        const value="B9";
        try {
         axios.delete('/wish/delete', { data: { key:key, value:value,username:user} });
          // Handle successful deletion
          console.log('Data deleted successfully.');
        } catch (error) {
          // Handle error
          console.error('Error deleting data:', error);
        }
      }
    }
    //Details-Bar:
    function baropen(imagesrc,weightitem,totalbill,id){
      const imgsrc = encodeURIComponent(imagesrc);
      const weight = weightitem;
      const itemtype = "GOLD";
      const itemid=id;
      const itemname="Gold Bangles"
      const total = totalbill;
      window.location.href=`/pricedetails/${imgsrc}/${weight}/${itemtype}/${total}/${itemname}/${itemid}`
    }
  return (
    <div>
      <Header/>
      <div className={searchstyle.search} onClick={Searchopen}>Search</div> 
        <div className={searchstyle.forsearch} style={{display:hide}}>
        <Search/>
        <img src="data:image/png;base64," id ="wrong" onClick={wrongsearch}/>
        </div> 
    <main style={{width:"100%",height:"550px", position:"absolute",top:"20px", filter:`blur(${blur})`}}>
    <div  className="orn-img"><img src="https://www.lalithaajewellery.com/assets/img/bangles/1.webp" alt=""   style={{width: "300px",height: "300px",  borderRadius: "20px"}} id="img1" />
  <img src={src1} className="svg"alt="" onClick={()=>starFn1("34.7","188828")} id="star1"/></div>
  <div className="orn-img"><img src="https://www.lalithaajewellery.com/assets/img/bangles/2.webp" alt=""  style={{width: "300px",height: "300px",  borderRadius: "20px"}} id="img2" /> <img src={src2} className="svg"alt=""  onClick={()=>starFn2("35.2","191041")} id="star2"/></div>
  <div className="orn-img"><img src=" https://www.lalithaajewellery.com/assets/img/bangles/3.webp" alt=""  style={{width: "300px",height: "300px",  borderRadius: "20px"}} id="img3" /> <img src={src3} className="svg"alt="" onClick={()=>starFn3("36.7","199182")} id="star3"/></div>
  <div className="orn-txt">
    Rs:<span id="cost1" className="orn-cost">188828</span>/-<br/>
    <span className="cal" id="vd1" onClick={()=>baropen("https://www.lalithaajewellery.com/assets/img/bangles/1.webp","34.7","188828","B1")}>View Details</span>
  </div>
  <div className="orn-txt">
    Rs:<span id="cost2"  className="orn-cost">191041</span>/-<br/>
    <span  className="cal" id="vd2"  onClick={()=>baropen("https://www.lalithaajewellery.com/assets/img/bangles/2.webp","35.2","191041","B2")}>View Details</span>
  </div>
  <div className="orn-txt">
    Rs:<span id="cost3"  className="orn-cost">199182</span>/-<br/>
    <span  className="cal" id="vd3" onClick={()=>baropen("https://www.lalithaajewellery.com/assets/img/bangles/3.webp","36.7","199182","B3")}>View Details</span>
  </div>
  <div className="orn-img"><img src="https://www.lalithaajewellery.com/assets/img/bangles/4.webp" alt=""   style={{width: "300px",height: "300px",  borderRadius: "20px"}}  id="img4"/> <img src={src4} className="svg"alt="" onClick={()=>starFn4("36.3","197510")} id="star4"/></div>
  <div className="orn-img"><img src="https://www.lalithaajewellery.com/assets/img/bangles/5.webp" alt=""    style={{width: "300px",height: "300px",  borderRadius: "20px"}}  id="img5"/> <img src={src5} className="svg"alt="" onClick={()=>starFn5("32.3","175302")} id="star5"/></div>
  <div className="orn-img"><img src="https://www.lalithaajewellery.com/assets/img/bangles/6.webp" alt=""   style={{width: "300px",height: "300px",  borderRadius: "20px"}}  id="img6"/> <img src={src6} className="svg"alt="" onClick={()=>starFn6("31.9","173631")} id="star6"/></div>
  <div className="orn-txt">
    Rs:<span id="cost4" className="orn-cost">197510</span>/- <br/>
    <span  className="cal" id="vd4" onClick={()=>baropen("https://www.lalithaajewellery.com/assets/img/bangles/4.webp","36.3","197510","B4")}>View Details</span>
  </div>
  <div className="orn-txt" >
    Rs:<span id="cost5"  className="orn-cost">175302</span>/- <br/>
    <span  className="cal" id="vd5" onClick={()=>baropen("https://www.lalithaajewellery.com/assets/img/bangles/5.webp","32.3","175302","B5")}>View Details</span>
  </div>
  <div className="orn-txt">
    Rs:<span id="cost6" className="orn-cost">173631</span>/-  <br/>
    <span  className="cal" id="vd6" onClick={()=>baropen("https://www.lalithaajewellery.com/assets/img/bangles/6.webp","31.9","173631","B6")}>View Details </span>
  </div>
  <div className="orn-img"><img src="https://www.lalithaajewellery.com/assets/img/bangles/7.webp" alt=""   style={{width: "300px",height: "300px",  borderRadius: "20px"}}  id="img7"/> <img src={src7} className="svg"alt="" onClick={()=>starFn7("35.6","193212")} id="star7"/></div>
  <div className="orn-img"><img src="https://www.lalithaajewellery.com/assets/img/bangles/11.webp" alt=""   style={{width: "300px",height: "300px",  borderRadius: "20px"}}  id="img8"/> <img src={src8} className="svg"alt="" onClick={()=>starFn8("37.8","205652")} id="star8"/></div>
  <div className="orn-img"><img src="https://www.lalithaajewellery.com/assets/img/bangles/12.webp" alt=""  style={{width: "300px",height: "300px",  borderRadius: "20px"}}  id="img9"/> <img src={src9} className="svg"alt="" onClick={()=>starFn9("35.21","191541")} id="star9"/></div>
  <div className="orn-txt">
    Rs:<span id="cost7" className="orn-cost">193212</span>/-  <br/>
    <span  className="cal" id="vd7" onClick={()=>baropen("https://www.lalithaajewellery.com/assets/img/bangles/7.webp","35.6","193212","B7")}>View Details</span>
  </div>
  <div className="orn-txt" >
    Rs:<span id="cost8"  className="orn-cost">205652</span>/-  <br/>
    <span  className="cal" id="vd8" onClick={()=>baropen("https://www.lalithaajewellery.com/assets/img/bangles/11.webp","37.8","205652","B8")}>View Details</span>
  </div>
  <div className="orn-txt">
    Rs:<span id="cost9"  className="orn-cost">191541</span>/-           <br/>
    <span  className="cal" id="vd9" onClick={()=>baropen("https://www.lalithaajewellery.com/assets/img/bangles/12.webp","35.21","191541","B9")}>View Details</span>
  </div>
    </main>
    </div>
  )
}

export default Bangles
